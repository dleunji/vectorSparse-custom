...
def get_file_name(bm, dimK, dimV, kernel, sort, job, sddmm_alg, precision, mem):
    ...
    if job == 'spmm':
        if mem:
            output_file = './csv/mem/spmm/%s_k%d_v%d.csv' % (bm.replace('', '').replace('.smtx', '') + suffix, dimK, dimV)
        else:
            output_file = './csv/spmm/%s_k%d_v%d.csv' % (bm.replace('', '').replace('.smtx', '') + suffix, dimK, dimV)
    # Else if the job is SDDMM
    else:
        if kernel == 'wmma':
            suffix += '_%s' % sddmm_alg
        if mem:
            output_file = './csv/mem/sddmm/%s_k%d_v%d.csv' % (bm.replace('', '').replace('.smtx', '') + suffix, dimK, dimV)
        else:
            output_file = './csv/sddmm/%s_k%d_v%d.csv' % (bm.replace('', '').replace('.smtx', '') + suffix, dimK, dimV)
    
    return output_file


def get_bm_size(bm, dimK, dimV):
    with open("dlmc/%s.smtx" % bm) as bm_file:
