...
bm_list = open('dlmc/%s_matrices.txt' % args.bm, 'r')
lines = bm_list.readlines()

def extract_duration_set(v, k, kernel, list_, alg):
    for i in np.arange(args.start, args.end):
        benchmark = lines[i][:-6]
        file_kernel = get_file_name('dlmc/%s' % benchmark, k, v, kernel, args.sort, 'sddmm', alg, 'half', False)
        file_dense = get_file_name('dlmc/%s' % benchmark, k, v, 'dense', args.sort, 'sddmm', 'None', 'half', False)
